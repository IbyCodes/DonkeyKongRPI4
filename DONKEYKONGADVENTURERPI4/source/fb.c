...
void myDrawImage(unsigned char * img, int width, int height, int offx, int offy, int dontDrawBlack) { // ADDED NEW argument for drawing black PIXELS (FOR DONKEY KONG)
    ...
    for (int i = 0; i < width; i++) for (int j = 0; j < height; j++) {
        ...
        if (dontDrawBlack == 1){
             if (argb_color != 0xFF000000 && argb_color != 0x00000000){
            myDrawPixel(x,y,argb_color);
             }
        }
        else{
        myDrawPixel(x,y,argb_color);// if we dont care about black pixels, just draw everything including black
    }
}
}
